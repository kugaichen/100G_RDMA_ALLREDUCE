...
module tb_moe_end_to_end_switch;
    ...
    task send_moe_packet;
        ...
        integer i;
        begin
            build_moe_first_beat(first, src_mac, src_ip, op_type, owner_rank, flags, seq, psn);
    ...
    integer packet_count [0:NUM_PORTS-1];
    integer dispatch_packet_count [0:NUM_PORTS-1];
    integer result_packet_count [0:NUM_PORTS-1];
    reg [7:0] packet_kind [0:NUM_PORTS-1];
    reg [31:0] packet_seq [0:NUM_PORTS-1];
    reg saw_dispatch [0:NUM_PORTS-1];
    reg saw_dispatch_seq0_exp0;
    reg saw_dispatch_seq0_exp1;
    reg saw_dispatch_seq1_exp0;
    reg saw_dispatch_seq1_exp1;
    reg saw_dispatch_seq2_exp0;
    reg saw_result_seq0_port0;
    reg saw_result_seq1_port1;
    reg saw_result_seq2_port0;
    reg saw_result_payload_seq0;
    reg saw_result_payload_seq1;
    reg saw_result_payload_seq2;
    integer evt_slot_dispatch_count;
    integer evt_dispatch_init_valid_count;
    integer evt_moe_init_valid_count;
    integer evt_moe_init_accept_count;
    integer evt_moe_data_valid_count;
    integer evt_moe_data_accept_count;
    ...
    initial begin
        ...
        saw_dispatch_seq0_exp0 = 1'b0;
        saw_dispatch_seq0_exp1 = 1'b0;
        saw_dispatch_seq1_exp0 = 1'b0;
        saw_dispatch_seq1_exp1 = 1'b0;
        saw_dispatch_seq2_exp0 = 1'b0;
        saw_result_seq0_port0 = 1'b0;
        saw_result_seq1_port1 = 1'b0;
        saw_result_seq2_port0 = 1'b0;
        saw_result_payload_seq0 = 1'b0;
        saw_result_payload_seq1 = 1'b0;
        saw_result_payload_seq2 = 1'b0;
        evt_slot_dispatch_count = 0;
        evt_dispatch_init_valid_count = 0;
        evt_moe_init_valid_count = 0;
        evt_moe_init_accept_count = 0;
        evt_moe_data_valid_count = 0;
        evt_moe_data_accept_count = 0;
        ...
        if (dut.u_allreduce_wrapper.u_allreduce.ENABLE_MOE_COMBINE != 1 ||
            ...
            $display("FAIL: MoE is disabled in nf_datapath allreduce core");
            $finish;
        end

        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        `MOE_OP_DISPATCH, 8'd0, 8'h03,
                        32'd0, 32'h0000_1000, 32'd3);
        wait (dispatch_packet_count[0] == 2);

        send_moe_packet(1, 8'h02, CFG_W1_MAC, CFG_W1_IP,
                        `MOE_OP_DISPATCH, 8'd1, 8'h03,
                        32'd1, 32'h0000_1001, 32'd5);
        wait (dispatch_packet_count[0] == 4);

        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        `MOE_OP_DISPATCH, 8'd0, 8'h01,
                        32'd2, 32'h0000_1002, 32'd8);

        wait (dispatch_packet_count[0] == 5);
        repeat (40) @(posedge clk);

        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        `MOE_OP_COMBINE_DATA, 8'd0, 8'h01,
                        32'd0, 32'h0000_2000, 32'd3);
        repeat (8) @(posedge clk);
        send_moe_packet(1, 8'h02, CFG_W1_MAC, CFG_W1_IP,
                        `MOE_OP_COMBINE_DATA, 8'd1, 8'h00,
                        32'd1, 32'h0000_2001, 32'd5);
        repeat (8) @(posedge clk);
        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        `MOE_OP_COMBINE_DATA, 8'd0, 8'h00,
                        32'd2, 32'h0000_2002, 32'd8);
        repeat (8) @(posedge clk);
        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        `MOE_OP_COMBINE_DATA, 8'd0, 8'h00,
                        32'd0, 32'h0000_2003, 32'd4);
        repeat (8) @(posedge clk);
        send_moe_packet(1, 8'h02, CFG_W1_MAC, CFG_W1_IP,
                        `MOE_OP_COMBINE_DATA, 8'd1, 8'h01,
                        32'd1, 32'h0000_2004, 32'd6);

        wait (saw_result_seq0_port0 && saw_result_payload_seq0 &&
              saw_result_seq1_port1 && saw_result_payload_seq1 &&
              saw_result_seq2_port0 && saw_result_payload_seq2);
        repeat (80) @(posedge clk);

        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        `MOE_OP_COMBINE_DATA, 8'd0, 8'h01,
                        32'd0, 32'h0000_2005, 32'd3);
        wait (dbg_drop_event_count == 32'd1);
        repeat (20) @(posedge clk);

        if (!saw_dispatch_seq0_exp0 ||
            !saw_dispatch_seq0_exp1 ||
            !saw_dispatch_seq1_exp0 ||
            !saw_dispatch_seq1_exp1 ||
            !saw_dispatch_seq2_exp0 ||
            !saw_result_seq0_port0 ||
            !saw_result_payload_seq0 ||
            !saw_result_seq1_port1 ||
            !saw_result_payload_seq1 ||
            !saw_result_seq2_port0 ||
            !saw_result_payload_seq2 ||
            dispatch_packet_count[0] != 5 ||
            dispatch_packet_count[1] != 0 ||
            result_packet_count[0] != 2 ||
            result_packet_count[1] != 1 ||
            result_packet_count[2] != 0 ||
            result_packet_count[3] != 0) begin
            $display("FAIL: packet counts dispatch_p0=%0d dispatch_p1=%0d result_p0=%0d result_p1=%0d result_p2=%0d result_p3=%0d flags d00=%0b d01=%0b d10=%0b d11=%0b d20=%0b r0=%0b r1=%0b r2=%0b",
                     dispatch_packet_count[0],
                     dispatch_packet_count[1],
                     result_packet_count[0],
                     result_packet_count[1],
                     result_packet_count[2],
                     result_packet_count[3],
                     saw_dispatch_seq0_exp0,
                     saw_dispatch_seq0_exp1,
                     saw_dispatch_seq1_exp0,
                     saw_dispatch_seq1_exp1,
                     saw_dispatch_seq2_exp0,
        ...
        if (dbg_dispatch_accept_count != 32'd3 ||
            dbg_init_accept_count != 32'd3 ||
            dbg_data_accept_count != 32'd5 ||
            dbg_release_accept_count != 32'd3 ||
            dbg_dispatch_output_count != 32'd5 ||
            dbg_combine_output_count != 32'd3 ||
        ...
        $display("TB_MOE_END_TO_END_SWITCH_PASS");
        $finish;
    end

    initial begin
        repeat (10000) @(posedge clk);
        $display("FAIL: timeout dispatch_p0=%0d dispatch_p1=%0d result_p0=%0d result_p1=%0d result_p2=%0d payloads=%0b%0b%0b counters da=%0d init=%0d data=%0d rel=%0d dout=%0d cout=%0d drop=%0d",
                 dispatch_packet_count[0],
                 dispatch_packet_count[1],
                 result_packet_count[0],
                 result_packet_count[1],
                 ...
                 dbg_combine_output_count,
                 dbg_drop_event_count);
        $display("DBG: events slot_dispatch=%0d dispatch_init_valid=%0d moe_init_valid=%0d moe_init_accept=%0d moe_data_valid=%0d moe_data_accept=%0d",
                 evt_slot_dispatch_count,
                 evt_dispatch_init_valid_count,
                 evt_moe_init_valid_count,
                 evt_moe_init_accept_count,
                 evt_moe_data_valid_count,
                 evt_moe_data_accept_count);
        $display("DBG: init_valid=%0b init_ready=%0b init_accept=%0b dispatch_init_valid=%0b dispatch_init_ready=%0b holding=%0b core_issue=%0b core_ready=%0b core_fire=%0b seen=%0b pending=%0b",
                 dut.u_allreduce_wrapper.u_allreduce.moe_init_valid,
                 dut.u_allreduce_wrapper.u_allreduce.moe_init_ready,
                 dut.u_allreduce_wrapper.u_allreduce.moe_init_accept,
                 dut.u_allreduce_wrapper.u_allreduce.moe_dispatch_init_valid,
                 dut.u_allreduce_wrapper.u_allreduce.moe_dispatch_init_ready,
                 dut.u_allreduce_wrapper.u_allreduce.u_moe_dispatch_init_adapter.holding_valid,
                 dut.u_allreduce_wrapper.u_allreduce.moe_core_slot_init_issue_valid,
                 dut.u_allreduce_wrapper.u_allreduce.moe_core_slot_init_ready,
                 dut.u_allreduce_wrapper.u_allreduce.moe_core_slot_init_fire,
                 dut.u_allreduce_wrapper.u_allreduce.moe_core_slot_init_seen,
                 dut.u_allreduce_wrapper.u_allreduce.moe_core_slot_init_pending);
        $display("DBG: slot_req valid=%0b kind=%0d dispatch=%0b owner=%0d seq=%0d flags=0x%02h payload_valid=%0b payload_op=0x%02h",
                 dut.u_allreduce_wrapper.u_allreduce.slot_req_valid,
                 dut.u_allreduce_wrapper.u_allreduce.slot_req_kind,
                 dut.u_allreduce_wrapper.u_allreduce.slot_req_is_dispatch,
                 dut.u_allreduce_wrapper.u_allreduce.slot_req_owner_rank,
                 dut.u_allreduce_wrapper.u_allreduce.slot_req_global_seq,
                 dut.u_allreduce_wrapper.u_allreduce.slot_req_flags,
                 dut.u_allreduce_wrapper.u_allreduce.parser_moe_payload_valid,
                 dut.u_allreduce_wrapper.u_allreduce.parser_moe_payload_op_type);
        $finish;
    end
                ...
                if (!rst_n) begin
                    ...
                    if (beat_count[gi] == 0) begin
                        ...
                        if (first_beat_kind == `MOE_OP_DISPATCH) begin
                            packet_seq[gi] <= port_tdata[511:480];
                            if (gi != 0 ||
                                port_tuser[31:24] !== 8'h01 ||
                                port_tdata[343:336] !== 8'h04 ||
                                port_tdata[447:432] !== `MOE_MAGIC ||
                                port_tdata[463:456] !== `MOE_OP_DISPATCH ||
                                (port_tdata[511:480] !== 32'd0 &&
                                 port_tdata[511:480] !== 32'd1 &&
                                 port_tdata[511:480] !== 32'd2) ||
                                ((port_tdata[511:480] == 32'd0) &&
                                 (port_tdata[471:464] !== 8'd0)) ||
                                ((port_tdata[511:480] == 32'd1) &&
                                 (port_tdata[471:464] !== 8'd1)) ||
                                ((port_tdata[511:480] == 32'd2) &&
                                 (port_tdata[471:464] !== 8'd0))) begin
                                $display("FAIL: dispatch output mismatch port=%0d data=0x%0128h tuser=0x%032h",
                                         gi, port_tdata, port_tuser);
                                $finish;
                            end
                            if (port_tdata[511:480] == 32'd0) begin
                                if (port_tdata[479:472] == 8'h01) begin
                                    saw_dispatch_seq0_exp0 <= 1'b1;
                                end
                                else if (port_tdata[479:472] == 8'h02) begin
                                    saw_dispatch_seq0_exp1 <= 1'b1;
                                end
                                else begin
                                    $display("FAIL: dispatch seq0 expert mask mismatch port=%0d data=0x%0128h",
                                             gi, port_tdata);
                                    $finish;
                                end
                            end
                            else if (port_tdata[511:480] == 32'd1) begin
                                if (port_tdata[479:472] == 8'h01) begin
                                    saw_dispatch_seq1_exp0 <= 1'b1;
                                end
                                else if (port_tdata[479:472] == 8'h02) begin
                                    saw_dispatch_seq1_exp1 <= 1'b1;
                                end
                                else begin
                                    $display("FAIL: dispatch seq1 expert mask mismatch port=%0d data=0x%0128h",
                                             gi, port_tdata);
                                    $finish;
                                end
                            end
                                ...
                                if (port_tdata[479:472] == 8'h01) begin
                                    saw_dispatch_seq2_exp0 <= 1'b1;
                                end
                                else begin
                                    $display("FAIL: dispatch seq2 expert mask mismatch port=%0d data=0x%0128h",
                                             gi, port_tdata);
                                    $finish;
                                end
                            end
                            ...
                            packet_seq[gi] <= port_tdata[511:480];
                            if (((port_tdata[511:480] == 32'd0) && (gi != 0 || port_tuser[31:24] !== 8'h01)) ||
                                ((port_tdata[511:480] == 32'd1) && (gi != 1 || port_tuser[31:24] !== 8'h02)) ||
                                ((port_tdata[511:480] == 32'd2) && (gi != 0 || port_tuser[31:24] !== 8'h01)) ||
                                (port_tdata[511:480] !== 32'd0 &&
                                 port_tdata[511:480] !== 32'd1 &&
                                 port_tdata[511:480] !== 32'd2) ||
                                port_tdata[343:336] !== 8'h04 ||
                                ...
                                ((port_tdata[511:480] == 32'd0) && (port_tdata[463:456] !== 8'd0)) ||
                                ((port_tdata[511:480] == 32'd1) && (port_tdata[463:456] !== 8'd1)) ||
                                ((port_tdata[511:480] == 32'd2) && (port_tdata[463:456] !== 8'd0))) begin
                                $display("FAIL: combine result header mismatch port=%0d data=0x%0128h tuser=0x%032h",
                                         gi, port_tdata, port_tuser);
                                $finish;
                            end
                            if (port_tdata[511:480] == 32'd0) begin
                                saw_result_seq0_port0 <= 1'b1;
                            end
                            else if (port_tdata[511:480] == 32'd1) begin
                                saw_result_seq1_port1 <= 1'b1;
                            end
                            else begin
                                saw_result_seq2_port0 <= 1'b1;
                            end
                        end
                        else begin
                            $display("FAIL: unexpected MoE output op=0x%02h port=%0d data=0x%0128h",
                                     first_beat_kind, gi, port_tdata);
                            $finish;
                        end
                    end
                    else if (beat_count[gi] >= 1 && beat_count[gi] <= 3) begin
                        if (packet_kind[gi] == `MOE_OP_COMBINE_RESULT &&
                            packet_seq[gi] == 32'd0 &&
                            gi == 0 &&
                            port_tdata[79:48] === 32'd7 &&
                            port_tdata[111:80] === 32'd7) begin
                            saw_result_payload_seq0 <= 1'b1;
                        end
                        else if (packet_kind[gi] == `MOE_OP_COMBINE_RESULT &&
                                 packet_seq[gi] == 32'd1 &&
                                 gi == 1 &&
                                 port_tdata[79:48] === 32'd11 &&
                                 port_tdata[111:80] === 32'd11) begin
                            saw_result_payload_seq1 <= 1'b1;
                        end
                        else if (packet_kind[gi] == `MOE_OP_COMBINE_RESULT &&
                                 packet_seq[gi] == 32'd2 &&
                                 gi == 0 &&
                                 port_tdata[79:48] === 32'd8 &&
                                 port_tdata[111:80] === 32'd8) begin
                            saw_result_payload_seq2 <= 1'b1;
                        end
                    end
                    ...
                    if (m_axis_tlast[gi]) begin
                        ...
                        packet_count[gi] <= packet_count[gi] + 1;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (dut.u_allreduce_wrapper.u_allreduce.slot_req_is_dispatch) begin
                evt_slot_dispatch_count <= evt_slot_dispatch_count + 1;
            end
            if (dut.u_allreduce_wrapper.u_allreduce.moe_dispatch_init_valid) begin
                evt_dispatch_init_valid_count <= evt_dispatch_init_valid_count + 1;
            end
            if (dut.u_allreduce_wrapper.u_allreduce.moe_init_valid) begin
                evt_moe_init_valid_count <= evt_moe_init_valid_count + 1;
            end
            if (dut.u_allreduce_wrapper.u_allreduce.moe_init_accept) begin
                evt_moe_init_accept_count <= evt_moe_init_accept_count + 1;
            end
            if (dut.u_allreduce_wrapper.u_allreduce.moe_data_valid) begin
                evt_moe_data_valid_count <= evt_moe_data_valid_count + 1;
            end
            if (dut.u_allreduce_wrapper.u_allreduce.moe_data_accept) begin
                evt_moe_data_accept_count <= evt_moe_data_accept_count + 1;
            end
        end
    end
endmodule
