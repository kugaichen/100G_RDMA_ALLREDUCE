...
module tb_moe_end_to_end_switch_1;
    ...
    reg saw_dispatch_owner0_expert0;
    reg saw_dispatch_owner0_expert1;
    reg saw_dispatch_owner1_expert1;
    reg saw_result_owner0;
    reg saw_result_payload_owner0;
    reg saw_result_owner1;
    reg saw_result_payload_owner1;
    ...
    initial begin
        ...
        saw_dispatch_owner0_expert0 = 1'b0;
        saw_dispatch_owner0_expert1 = 1'b0;
        saw_dispatch_owner1_expert1 = 1'b0;
        saw_result_owner0 = 1'b0;
        saw_result_payload_owner0 = 1'b0;
        saw_result_owner1 = 1'b0;
        saw_result_payload_owner1 = 1'b0;
        ...
        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        ...
                        32'd0, 32'h0000_1000, 32'h1122_3344);
        ...
        send_moe_packet(0, 8'h01, CFG_W0_MAC, CFG_W0_IP,
                        ...
                        32'd0, 32'h0000_1001, 32'd2);
        repeat (20) @(posedge clk);
        send_moe_packet(1, 8'h02, CFG_W1_MAC, CFG_W1_IP,
                        `MOE_OP_COMBINE_DATA, 8'd0, 8'h01,
                        32'd0, 32'h0000_1002, 32'd3);

        wait (saw_result_owner0 && saw_result_payload_owner0);
        repeat (100) @(posedge clk);

        send_moe_packet(1, 8'h02, CFG_W1_MAC, CFG_W1_IP,
                        `MOE_OP_DISPATCH, 8'd1, 8'h02,
                        32'd1, 32'h0000_2000, 32'h5566_7788);

        wait (dispatch_packet_count[0] == 3);
        repeat (50) @(posedge clk);

        send_moe_packet(1, 8'h02, CFG_W1_MAC, CFG_W1_IP,
                        `MOE_OP_COMBINE_DATA, 8'd1, 8'h01,
                        32'd1, 32'h0000_2001, 32'd4);

        wait (saw_result_owner1 && saw_result_payload_owner1);
        repeat (100) @(posedge clk);

        if (!saw_dispatch_owner0_expert0 ||
            !saw_dispatch_owner0_expert1 ||
            !saw_dispatch_owner1_expert1 ||
            !saw_result_owner0 ||
            !saw_result_payload_owner0 ||
            !saw_result_owner1 ||
            !saw_result_payload_owner1 ||
            dispatch_packet_count[0] != 3 ||
            ...
            $display("FAIL: packet counts dispatch_p0=%0d dispatch_p1=%0d result_p0=%0d result_p1=%0d result_p2=%0d result_p3=%0d flags d00=%0b d01=%0b d11=%0b r0=%0b r0p=%0b r1=%0b r1p=%0b",
        ...
        if (dbg_dispatch_accept_count != 32'd2 ||
            ...
            dbg_data_accept_count != 32'd3 ||
            dbg_release_accept_count != 32'd2 ||
            dbg_dispatch_output_count != 32'd3 ||
    ...
    initial begin
        ...
        $display("FAIL: timeout dispatch_p0=%0d dispatch_p1=%0d result_p0=%0d result_payload=%0b counters da=%0d init=%0d data=%0d rel=%0d dout=%0d cout=%0d drop=%0d",
                 dispatch_packet_count[0],
                 dispatch_packet_count[1],
                 result_packet_count[0],
                 saw_result_payload_owner0 | saw_result_payload_owner1,
                 dbg_dispatch_accept_count,
                 dbg_init_accept_count,
                 dbg_data_accept_count,
                 dbg_release_accept_count,
                ...
                if (!rst_n) begin
                    ...
                    if (beat_count[gi] == 0) begin
                        ...
                        if (first_beat_kind == `MOE_OP_DISPATCH) begin
                            ...
                            if (port_tdata[511:480] == 32'd0) begin
                                if (port_tdata[479:472] == 8'h01) begin
                                    saw_dispatch_owner0_expert0 <= 1'b1;
                                end
                                else if (port_tdata[479:472] == 8'h02) begin
                                    saw_dispatch_owner0_expert1 <= 1'b1;
                                end
                                else begin
                                ...
                                if (port_tdata[479:472] == 8'h02) begin
                                    saw_dispatch_owner1_expert1 <= 1'b1;
                                end
                                else begin
                        ...
                        if (packet_kind[gi] == `MOE_OP_COMBINE_RESULT &&
                            packet_seq[gi] == 32'd0 &&
                            gi == 0 &&
                            port_tdata[79:48] === 32'd5 &&
                            port_tdata[111:80] === 32'd5) begin
                            saw_result_payload_owner0 <= 1'b1;
                        end
                        else if (packet_kind[gi] == `MOE_OP_COMBINE_RESULT &&
                                 packet_seq[gi] == 32'd1 &&
                                 gi == 1 &&
                                 port_tdata[79:48] === 32'd4 &&
                                 port_tdata[111:80] === 32'd4) begin
                            saw_result_payload_owner1 <= 1'b1;
                        end
                    end
